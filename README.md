...
| `rolebyte` | `rolebyte` | Central administration of tenants, user accounts, the per-service role vocabulary and the permissions each service declares, each tenant's own roles made of those permissions, what each tenant has of that catalog, and role assignments, with an append-only change history. |
